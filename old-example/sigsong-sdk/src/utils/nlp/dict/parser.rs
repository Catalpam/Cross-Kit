...
fn parse_header(input: &[u8]) -> IResult<&[u8], Header> {
    ...
    let info = UTF_16LE.decode(info, encoding::DecoderTrap::Strict).unwrap();
    let attrs = parse_key_value(info.as_str());

    let version =
        attrs.get("GeneratedByEngineVersion").unwrap().trim().slice(0..1).parse::<u8>().unwrap();
    ...
    let encoding = attrs.get("Encoding").unwrap_or(&"UTF-8".to_string()).to_string();

    Ok((input, Header { version, encrypted, encoding }))
}
...
fn parse_key_block_infos_v2<'a>(
    ...
    if dict_header.encrypted == 2 {
        ...
        d.extend(decrypte);
        zlib::Decoder::new(&d[8..]).read_to_end(&mut key_block_info).unwrap();
    }
...
fn parse_block_items_v2<'a>(
    ...
    let (remain, sep) =
        many0(map(tuple((be_u64, take_till(|x| x == 0), take(1_usize))), |(offset, buf, _)| {
            let decoder = encoding_from_whatwg_label(encoding).unwrap();
            let text = decoder.decode(buf, encoding::DecoderTrap::Ignore).unwrap();
            KeyEntry { offset: offset as usize, text }
        }))(input)?;
...
fn block_parser_v1<'a>(size: usize) -> impl FnMut(&'a [u8]) -> IResult<&'a [u8], Vec<u8>> {
    map(tuple((le_u32, take(4_usize), take(size - 8))), |(enc, chksum, encrypted)| {
        ...
        let data: Vec<u8> = match enc_method {
            ...
            2 => {
                let decrypt = vec![];
                let cipher = Salsa20::new(key.as_slice().into(), &[0; 8].into());
...
fn block_parser<'a>(
    ...
    map(tuple((le_u32, take(4_usize), take(comp_size - 8))), move |(enc, chksum, encrypted)| {
        let enc_method = (enc >> 4) & 0xf;
        let enc_size = (enc >> 8) & 0xff;
        let comp_method = enc & 0xf;
...
fn parse_record_blocks_v1(input: &[u8]) -> IResult<&[u8], Vec<BlockEntryInfo>> {
    ...
    count(
        map(tuple((be_u32, be_u32)), |(compressed_size, decompressed_size)| BlockEntryInfo {
            compressed_size: compressed_size as usize,
            decompressed_size: decompressed_size as usize,
        }),
        records as usize,
...
fn parse_record_blocks_v2(input: &[u8]) -> IResult<&[u8], Vec<BlockEntryInfo>> {
    ...
    count(
        map(tuple((be_u64, be_u64)), |(compressed_size, decompressed_size)| BlockEntryInfo {
            compressed_size: compressed_size as usize,
            decompressed_size: decompressed_size as usize,
        }),
        records as usize,
...
pub(crate) fn record_block_parser<'a>(
    ...
    map(tuple((le_u32, take(4_usize), take(size - 8))), move |(enc, chksum, encrypted)| {
        let enc_method = (enc >> 4) & 0xf;
        let enc_size = (enc >> 8) & 0xff;
        let comp_method = enc & 0xf;
...
pub fn parse(data: &[u8]) -> Mdx {
    ...
    let (input, key_blocks) =
        parse_key_blocks(input, key_block_header.key_block_size, &header, &key_block_infos)
            .unwrap();
