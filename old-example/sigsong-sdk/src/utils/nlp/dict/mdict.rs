...
impl Mdx {
    pub fn items(&self) -> impl Iterator<Item = Record> {
        self.key_blocks.iter().flat_map(|block| &block.entries).map(|entry| {
            let def = self.find_definition(entry);
            Record { key: &entry.text, definition: def }
    ...
    pub(crate) fn record_offset(&self, entry: &KeyEntry) -> Option<RecordOffset> {
    ...
    pub fn find_definition(&self, entry: &KeyEntry) -> String {
