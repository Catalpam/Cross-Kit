...
mod tests {
    ...
    fn writes_swift_bridge_files_from_ir_codegen() {
        ...
        assert!(content.contains("public func incrementBy(deltaValue: Int32) -> CounterState"));
        assert!(!content.contains("public func getState"));
        assert!(!content.contains("public func subscribe"));
        assert!(!content.contains("public func unsubscribe"));
        assert!(!content.contains("public func __crossKitProbe"));
        assert!(!content.contains("/// Calls the Rust VM `on_state` action."));
        assert!(!content.contains("vm.onState("));
    ...
    fn counter_ir() -> VmMetadata {
        serde_json::from_value(serde_json::json!({
            ...
            "methods": [
                ...
                {
                    "name": "increment_by",
                    "args": [{"name": "delta_value", "rust_type": "i32"}],
                    "return_type": "CounterState"
                }
            ]
        }))
