...
fn generate_state_bridge(meta: &VmMetaLocal) -> String {
    let ctor = constructor_method(meta);
    let ctor_sig = ctor
        .map(|method| format_swift_signature(method, true))
        .unwrap_or_else(|| "public init()".to_string());
    let ctor_call = ctor
        .map(|method| format_swift_ctor_call(method, &meta.vm_type))
        .unwrap_or_else(|| format!("{}()", meta.vm_type));
...
"#,
        ...
        observer_method = to_swift_method_name(&meta.observer_method),
        ctor_sig = ctor_sig,
        ctor_call = ctor_call,
