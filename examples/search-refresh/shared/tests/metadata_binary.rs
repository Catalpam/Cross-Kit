...
fn run_metadata_binary() -> String {
    let output = Command::new(env!("CARGO_BIN_EXE_ck_search_refresh_metadata"))
        .output()
...
#[test]
fn metadata_contains_search_refresh_contract() {
    let json = run_metadata_binary();
    let parsed: Value = serde_json::from_str(json.trim()).unwrap();
    ...
    assert_eq!(ir.rust_type, "SearchViewModel");
    assert_eq!(ir.bridge_name, "SearchViewModelBridge");
    assert_eq!(ir.state_type.as_deref(), Some("SearchState"));
    ...
    assert!(!methods.iter().any(|method| method.contains("async")));
    assert!(!methods.iter().any(|method| method.contains("task")));
}

#[test]
fn metadata_ir_matches_search_refresh_snapshot() {
    let json = run_metadata_binary();
    let parsed: Value = serde_json::from_str(json.trim()).unwrap();
    ...
    let expected_ir: Vec<Value> = serde_json::from_str(include_str!(
        "../../../../fixtures/metadata/search-refresh.json"
    ))
    .unwrap();
...
fn metadata_contains_swift_bridge_code() {
    ...
    assert!(swift_code.contains("public final class SearchViewModelBridge"));
    assert!(swift_code.contains("public func updateQuery("));
    assert!(swift_code.contains("public func submit()"));
    assert!(swift_code.contains("public func tick()"));
    assert!(swift_code.contains("public func cancel()"));
}
